...
public class GameServerService : ServiceBase
{
	...
	public async Task<string> StartGameServer(long placeId, int RCCPort, int NSPort, string jobId, int JobExpiration, long MaxPlayers)
	{
		...
		rccServer.StartInfo.FileName = @"C:\bubbablox-v2-2021\GameServer16\RCCService.exe";
		...
		string XML = $@"<?xml version=""1.0"" encoding=""utf-8""?>
			<soap:Envelope xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance""
			   ...
			   xmlns:soap=""http://schemas.xmlsoap.org/soap/envelope/"">
				<soap:Body>
					<OpenJobEx xmlns=""http://novarbx.cc/"">
						<job>
							<id>{jobId}</id>
							<category>1</category>
							<cores>1</cores>
	...
	public static async Task<bool> SendSoapRequestToRcc(string URL, string XML, string SOAPAction, int maxRetries = 5)
	{
		...
		for (int attempt = 1; attempt <= maxRetries; attempt++)
		{
			using (HttpClient RccHttpClient = new HttpClient())
			{
				...
				RccHttpClient.DefaultRequestHeaders.Add("SOAPAction", $"http://novarbx.cc/{SOAPAction}");
		...
		for (int attempt = 1; attempt <= maxRetries; attempt++)
		{
			try
			{
				using (var client = new HttpClient())
				{
					...
					request.Headers.Add("SOAPAction", $"http://novarbx.cc/{action}");
...
/* 				Script = $@"{{
			...
			<soap:Envelope xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance""
			   ...
			   xmlns:soap=""http://schemas.xmlsoap.org/soap/envelope/"">
				<soap:Body>
					<{SoapAction} xmlns=""http://novarbx.cc/"">
						<job>
