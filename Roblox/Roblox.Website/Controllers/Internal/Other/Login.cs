    ...
    {		
		...
		public async Task<MVC.IActionResult> DiscordVerify([MVC.FromQuery] string code)
		{
			try
			{
				...
				{
					return Redirect("/?signupmsg=This Discord account is already linked to another NOVARBX account.");
				}
		...
		[MVC.HttpGet("api/auth/validate-invite")]
		public async Task<MVC.IActionResult> ValidateInvite(
			[MVC.FromServices] NpgsqlConnection db,
			[MVC.FromQuery] string key)
		{
			if (string.IsNullOrWhiteSpace(key))
			{
				// Use the explicit JsonResult object instead of the Json() helper method
				return new Microsoft.AspNetCore.Mvc.JsonResult(new { valid = false });
			}

			var count = await db.QueryFirstOrDefaultAsync<int>(
				"SELECT COUNT(*) FROM invite_keys WHERE key = @key",
				new { key = key }
			);

			return new Microsoft.AspNetCore.Mvc.JsonResult(new { valid = count > 0 });
		}
        ...
        public async Task<MVC.IActionResult> Signup(
            ...
            [MVC.FromForm] string username,
            [MVC.FromForm] string password,
            [MVC.FromForm] string inviteKey, // <-- Added parameter matching the form input
            ...
            [MVC.FromForm] bool isEligibleForHideAdsAbTest = false)
        {
            ...
            if (string.IsNullOrWhiteSpace(inviteKey))
                return ReturnFormError("Invite key is required", 
                    new List<string> { "InviteKeyRequired" });
            ...
            if (!await services.users.IsUsernameValid(username))
                return ReturnFormError("Invalid Username", 
            ...
            if (!services.users.IsPasswordValid(password))
                return ReturnFormError("Invalid password", 
            ...
            try
            {
                ...
                // --- KEY CONSUMPTION ---
                // We delete the key right here. If rowsAffected is 0, the key didn't exist or was just snatched by another request.
                // We do this right before user creation so that early validation errors (like taken usernames) don't burn the key unnecessarily.
                var rowsAffected = await db.ExecuteAsync(
                    "DELETE FROM invite_keys WHERE key = @key",
                    new { key = inviteKey }
                );

                if (rowsAffected == 0)
                {
                    return ReturnFormError("Invalid or already used invite key.", 
                        new List<string> { "InvalidInviteKey" });
                }

                // Create the user now that the key is validated and consumed
