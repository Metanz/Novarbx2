...
- ```"Postgres": "Host=127.0.0.1; Database=NovaRBXnew; Password=test; Username=postgres; Maximum Pool Size=20",```
...
- press ```CTRL + H``` and change ```C:\\Users\\Admin\\Desktop\\Revival\\ecsr\\ecsrev-main\\services\\``` to ```C:\\whereever your NOVARBX folder is\\``` (make sure it's double slashed! so it should look like ```C:\\folder1\\folder2\\```)
